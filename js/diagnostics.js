...
var diagnosticsArrayStorageNames = new Set(['goals','tasks','time','habits','habitLogs','journal','events','flashcards','cardReviewDetails','focusSessions','contacts','quotes','decisions','doctrineLogs','doctrineCustomModules','doctrineLinks','scenarioHistory','practiceQueue','learningCycles','captureInbox','entityLinks','profileFeedback','qualityReviews']);
var diagnosticsObjectStorageNames = new Set(['settings','cardReviews','weeklyReviews','discipline','protocol','learnProgress','pmpProgress','pomodoroStats','strategicHorizon','frameworkLab','doctrineSourcesState','sessionContext','intel','commitmentPrefs','practiceQuality','commTracker','energyTracker','principlesTracker','cebokTracker','doctrineDomainOverrides','doctrineModelOverrides','adaptiveProfile']);
...
function diagnosticsCheckStorageEntry(entry){
  ...
  try {
    ...
    if (diagnosticsArrayStorageNames.has(entry.name) && !Array.isArray(parsed)) {
      return { ok: false, detail: 'Expected an array, found ' + (parsed === null ? 'null' : typeof parsed) };
    }
    if (diagnosticsObjectStorageNames.has(entry.name) && (parsed === null || Array.isArray(parsed) || typeof parsed !== 'object')) {
