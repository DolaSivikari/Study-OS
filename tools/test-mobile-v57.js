...
check('Hamburger exposes drawer ARIA state',
  /aria-controls="primarySidebar"/.test(index) && /aria-expanded="false"/.test(index));
check('Primary sidebar has a stable controlled ID', /id="primarySidebar"/.test(index));
...
check('Mobile bottom navigation still contains six destinations',
  (index.match(/<nav class="mobile-nav"[\s\S]*?<\/nav>/) || [''])[0].match(/<button/g)?.length === 6);
