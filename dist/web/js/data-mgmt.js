...
function validateSnapshotData(data){
  ...
  const shapes = window.STUDYOS_STORAGE_SHAPES || {};
  known.forEach(name => {
    const value = data[name];
    if (value === null || value === undefined) return;
    if (shapes[name] === 'array' && !Array.isArray(value)) throw new Error('Backup key "' + name + '" must be an array');
    if (shapes[name] === 'object' && (typeof value !== 'object' || Array.isArray(value))) throw new Error('Backup key "' + name + '" must be an object');
  });
